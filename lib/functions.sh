...

{
    ...
    function touch_github() {
        ssh -o BatchMode=yes -o ConnectTimeout=5 -o IdentitiesOnly=yes -F /dev/null -i coinmenode -i coinmenode.pub -T git@github.com || SSH_GIT_EXIT_CODE=$?

        if [[ ${SSH_GIT_EXIT_CODE} -eq 1 ]]; then
            echo "successfully tested coinmenode against github.com"
        else
            echo "FAILED: github rejected the coinmenode|coinmenode.pub keys!"
            exit ${SSH_GIT_EXIT_CODE}
        fi
    }
    ...
    # call via: getFolderName git@github.com:coinme/coinme-kiosk.git
